...
public class CloudSimExample6 {
	...
	public static void main(String[] args) {
		Log.printLine("Starting CloudSimExample6..adasd.");
		...
		try {
			...
			cloudletList = createCloudlet(brokerId, 5000); // creating 40 cloudlets
	...
	private static void GetData(List<Vm> vmlist) {

		double totalCPU = 0;
		double utilizedCPU = 0;
		double maxPower = 0;
		double minPower = 0;
		double totalMem = 0;
		double utilizedMem = 0;
		double totalBW = 0;
		double utilizedBw = 0;

		for (Host host : hostList) {

			totalMem = host.getRam();
			host.getVmList().size();
			Vm vm = vmlist.get(0);
			host.utilizedMem = vm.getRam() * vmlist.size();
			host.utilizedBw = vm.getBw() * vmlist.size();
			host.utilizedCPU = vm.getMips() * vmlist.size();
			host.minPower = host.maxPower;
			host.maxPower = host.maxPower * vmlist.size();
		}
		for (Host host : hostList) {
			host.sla = getSla(host.utilizedBw, host.getBw());
			host.ResourceRemainingRate = getResourceRemainingRate(host.utilizedCPU, host.getTotalMips(),
					host.utilizedMem, host.getRam(), host.utilizedBw, host.getBw());
			host.PowerConsumption = getPowerConsumption(host.utilizedCPU, host.getTotalMips(), host.minPower,
					host.maxPower);
		}

		for (Host host : hostList) {

			Log.printLine(" Host" + host.getId() + " : sla is..." + host.sla);
			Log.printLine(" Host" + host.getId() + " : ResourceRemainingRate is..." + host.ResourceRemainingRate);
			Log.printLine(" Host" + host.getId() + " : PowerConsumption is..." + host.PowerConsumption);
		}
	...
	public static double getResourceRemainingRate(double cpu_utilized, double cpu_total, double mem_utilized,
			double mem_total, double bw_utilized, double bw_total) {
